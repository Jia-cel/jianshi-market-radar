...
router.post('/analyze', authRequired, async (req, res) => {
  ...
  try {
    ...
    console.error('[pattern] OpenAI error:', detail);
    // 检查常见错误
    let hint = '';
    if (detail.includes('401') || detail.includes('Incorrect API key')) hint = ' (API Key 无效)';
    else if (detail.includes('429') || detail.includes('quota')) hint = ' (配额不足，需充值)';
    else if (detail.includes('model')) hint = ' (模型不可用，请检查模型名)';
    else if (detail.includes('billing')) hint = ' (账户余额不足)';
